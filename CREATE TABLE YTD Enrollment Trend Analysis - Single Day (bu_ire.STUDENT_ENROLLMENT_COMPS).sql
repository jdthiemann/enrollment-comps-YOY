/*	Drop existing table */
--DROP TABLE bu_ire.STUDENT_ENROLLMENT_COMPS;

/*	Delete data from table */
--DELETE FROM bu_ire.STUDENT_ENROLLMENT_COMPS;
...
CREATE TABLE bu_ire.STUDENT_ENROLLMENT_COMPS
	(
		ENRCOMPS_SINGLE_ID INT IDENTITY(1,1) NOT NULL
		...
		, CONSTRAINT PK_ENRCOMPS_SINGLE_ID PRIMARY KEY (ENRCOMPS_SINGLE_ID ASC)
		WITH (PAD_INDEX = OFF, STATISTICS_NORECOMPUTE = OFF, IGNORE_DUP_KEY = OFF, ALLOW_ROW_LOCKS = ON, ALLOW_PAGE_LOCKS = ON) ON [PRIMARY]
	) ON [PRIMARY]
GO

/*	Create table indices */
CREATE INDEX IDX_TERM ON bu_ire.STUDENT_ENROLLMENT_COMPS (TERM);
CREATE INDEX IDX_TERM_SEQ ON bu_ire.STUDENT_ENROLLMENT_COMPS (TERM_SEQ);
CREATE INDEX IDX_BATES_STAMP on bu_ire.STUDENT_ENROLLMENT_COMPS (BATES_STAMP);
CREATE INDEX IDX_STTR_STUDENT ON bu_ire.STUDENT_ENROLLMENT_COMPS (STTR_STUDENT);
		...
		FROM
			(
				SELECT
					...
					, T.TERM_START_DATE
					, T.TERM_END_DATE
					, DATEDIFF(DAY, T.TERM_START_DATE, GETDATE()) AS PIVOT_DATE
					, ROW_NUMBER() OVER(PARTITION BY
							...
							WHEN 'SU' THEN 'Summer'
						END
							ORDER BY ABS(DATEDIFF(DAY, T.TERM_START_DATE, GETDATE()))) AS DAY_DIF
...
INSERT INTO bu_ire.STUDENT_ENROLLMENT_COMPS
...
FROM
	PIVOT_CTE T
	LEFT OUTER JOIN bu_ire.STUDENT_ENROLLMENT_YTD Y ON T.TERM_SEASON=Y.TERM_SEASON AND T.PIVOT_DATE=Y.BATES_STAMP
...
SELECT *
FROM bu_ire.STUDENT_ENROLLMENT_COMPS
WHERE STTR_STUDENT IN (2082986, 2082989) AND TERM = '2018FA'
ORDER BY 6, 10, 9

/*	Revision history:
		JDT 7/29/2020: Added the "NO OFFSET" version of the credit summary for a given day.
		JDT 6/8/2020: Added latest term and latest term description for use in comparative enrollment reports.
		...
		JDT 7/29/2019: Added primary academic level.
		JDT 7/22/2019: Created view to list only today's enrollment comparison data based on the current enrollment period(s).
*/
